...
try {
    ...
    $almacenElaborado = AlmacenElaboracionesDao::select($id);

    $idReceta = $almacenElaborado->getRecetaId();


    // Obtener la receta por ID
    $receta = RecetasDao::select($idReceta);

    // Obtener los ingredientes y elaborados asociados
    $listIngs = RecetaIngredienteDao::getIngredientesByRecetaId($idReceta);
    $listElaborados = RecetaElaboradoDao::getElaboradosByRecetaId($idReceta);
    ...
    $response = [
        ...
        "almacenElaborado" =>[
            "id" => $almacenElaborado->getID(),
            "nombre" => $almacenElaborado->getFName(),
            "fecha" => $almacenElaborado->getFechaElab(),
            "caducidad" => $almacenElaborado->getFechaCad(),
            "cantidad" => $almacenElaborado->getProductamount(),
            "unidades_paquete" => $almacenElaborado->getRationsPackage(),
            "costo" => $almacenElaborado->getCostPrice(),
            "moneda_costo" => $almacenElaborado->getCostCurrency(),
            "precio_venta" => $almacenElaborado->getSalePrice(),
            "moneda_venta" => $almacenElaborado->getSaleCurrency(),
            "codigo_barras" => $almacenElaborado->getCodeContents(),
            "estado" => $almacenElaborado->getEstado(),
            "tipo_producto" => $almacenElaborado->getTipoProd(),
            "empaquetado" => $almacenElaborado->getPackaging(),
            "almacen" => $almacenElaborado->getWarehouse(),
            "nombre_archivo" => $almacenElaborado->getFileName()
        ],
        "receta" => [
            "id" => $receta->getId(),
