...
<body id="page-top">
    ...
    <script src="https://cdn.jsdelivr.net/npm/sweetalert2@11"></script>
      <script type="module">
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (response.status === 'success') {
                        let ingredientes = filtrarPorIng(response.data);
                        displayProducts(ingredientes);
        ...
        function filtrarPorIng(products) {
            // Usamos el método filter para filtrar los productos
            return products.filter(product => product.sku.startsWith('ING'));
        }
        ...
        function displayProducts(products) {
            ...
            products.forEach(product => {
                const isMainProduct = !product.sku.match(/-\d+-L\d+$/);
                ...
                if (isMainProduct) {
                    ...
                    productsHtml += `<p style="display: none;"><strong>Fecha de Elaboración:</strong> ${product.fecha_elaboracion}</p>`;
                    productsHtml += `<p style="display: none;"><strong>Fecha de Caducidad:</strong> ${product.fecha_caducidad}</p>`;


                    if (product.variations && product.variations.length > 0) {
                        productsHtml += `<p><strong>Variación:</strong>`;
                        productsHtml += `<select class="variation-select" data-product-id="${product.id}">`;
                        productsHtml += `<option value="">Select a variation</option>`;

                        product.variations.forEach(variation => {
                            productsHtml += `<option value="${variation.id}" data-stock="${variation.stock_quantity}" data-name="${variation.name}">
                                            ${variation.name} - Precio: ${variation.price}
                                          </option>`;
                        });
                        productsHtml += `</select></p>`;

                        productsHtml += `<p><strong>Stock: </strong><span class="stock-info" id="stock-${product.id}">Select a variation</span></p>`;
                    }

                    productsHtml += `
                    <div class="quantity-control">
                        <button class="btn-decrement" data-product-id="${product.id}">-</button>
                        <input type="number" class="quantity-input" data-product-id="${product.id}" value="0" min="0">
        ...
        function setupEventListeners() {
            $('.variation-select').change(function() {
                const selectedVariation = $(this).find('option:selected');
                const stock = selectedVariation.data('stock');
                const productId = $(this).data('product-id');
                const inputField = $(`.quantity-input[data-product-id="${productId}"]`);
                inputField.val(parseInt(0));
                $(`#stock-${productId}`).text(stock || 'No disponible');
            });

            $('.btn-increment').click(function() {
                const productId = $(this).data('product-id');
                const selectedVariation = $(`.variation-select[data-product-id="${productId}"]`).find('option:selected');
                const stock = selectedVariation.data('stock') || 0; // Stock de la variación seleccionada
        ...
        $('#addStockButton').click(function() {
            ...
            $('.product-card').each(function() {
                ...
                let variationId = $(this).find('.variation-select').val();
                ...
                let selectedOption = $(this).find('.variation-select option:selected');
                let dataNameSelect = selectedOption.data('name') || ''; // Valor por defecto vacío si no existe
                ...
                if (quantity > 0 && variationId) {
                    ...
                    console.log("sku:", sku);
                    console.log("dataNameSelect:", dataNameSelect);
                }
            });
            ...
            if (selectedProducts.length > 0) {
                ...
                $.ajax({
                    url: `${BASE_URL}/ecommerce/apiwoo/updateStockLote.php`,
                    type: 'POST',
                    data: {
                    ...
                    success: function(response) {
                        ...
                        if (response.status === 'success') {
                            ...
                            $.ajax({
                                url: 'guardarLotes.php', // Archivo PHP que guardará los lotes en la base de datos
                                type: 'POST',
                                data: {
                                    lotes: JSON.stringify(response.data)
                                }, // Enviar los lotes como JSON
                                success: function(saveResponse) {
                                    console.log(saveResponse); // Ver la respuesta de la operación de guardado
                                },
