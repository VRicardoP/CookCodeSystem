				...
				if ( '_local_pickup_time_select' == $nameField ) {
					...
					if ( is_array( $meta->get_data()['value'] ) ) {
						...
						if ( is_string( $meta->get_data()['value'] ) ) {
							...
							$time  = 'hfd_ship_number' !== $meta->get_data()['key'] && '_billing_wooccm11' !== $meta->get_data()['key'] ? strtotime( $value ) : false;

							// Check for Documento Nacional de Identidad, cause it might look like a date string,
							// And strtotime might return a value
							$is_dni = str_contains( strtolower( $nameField ), 'dni' );
							if ( ! $is_dni && $time ) {
								$is_date = strtotime( $value ) !== false;
								if ( $is_date ) {
									$date_format                = date_i18n( wc_date_format(), strtotime( $value ) );
